...
start = config['target']['start']
end = config['target']['end']

# 幅を広くする
st.set_page_config(layout="wide")

# 冒頭の説明いろいろ
st.title(':face_with_monocle:  議会見える化プロジェクト(β)@' + place)
st.markdown('(オリジナル版（[中央区版](https://mieruca-chuo.glide.page/dl/6471c6)）のタイトルメッセージより引用)')
...
giin_list_temp = pd.read_csv(config['giin'], encoding='UTF-8')
giin_list = giin_list_temp['氏名'] # 氏名だけを抜き出したdataframeを作る

# 委員会リストCSVの読み込み（議事録csvから作れるはずだがやってない
iinkai_list_temp = pd.read_csv(config['iinkai'], encoding='UTF-8')
...
with st.expander("■「年度」での絞り込み", False):# 折りたたみ
  ...
  '最近の動向を知りたいとか、対象の年度で結果を絞りたい場合は使ってみてください。初期値では全部の年度が選択されてます。',
    options=year_list, # 選択肢として表示する年
    value=(first_year, last_year)) # 初期値
  st.markdown('　※ 政治家を選択せずに絞り込みを設定すると勝手に人が変わっちゃいます。その場合は政治家を選択してください。')
...
if len(logs_contents) == 0:
  ...
  text = open("work/temp.txt", encoding="utf8").read()
  ...
  stpwds_temp = pd.read_csv(config['jogai'], header=0, encoding='UTF-8')
  stpwds = list(stpwds_temp['除外語'])
  stpwds.append(place)
  ...
  wc.generate(words)
  wc.to_file('work/wc.png') # ファイルとして保存
  st.image('work/wc.png') # 画像の表示
...
st.markdown('分析の元になっているデータは、[' + place +'議会 Webサイト](' + config['target']['url']+ ')の「会議録検索」からHTMLファイルをごっそりダウンロードして、その上であれこれ苦心して加工して作成しました。注意して作業はしたつもりですが、一部のデータが欠損等している可能性もありますのでご承知おきください。もし不備等ありましたら['+config['developer']['name']+']('+config['developer']['contact']+')まで声掛けいただけるとありがたいです。')

# 画像の読み込み
st.header(':face_with_monocle: 作成者情報')
image = Image.open('static/original.png')
st.image(image,width=100)
st.markdown('**オリジナル版（[中央区版](https://mieruca-chuo.glide.page/dl/6471c6)）を作った人：[ほずみゆうき](https://twitter.com/ninofku)**')
image = Image.open(config['developer']['img'])
st.image(image,width=100)
st.markdown('**本サイト（' + place + '版）を作った人：[' + config['developer']['name'] + '](' + config['developer']['contact'] + ')**')